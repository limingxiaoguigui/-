 ...
 */
const getUserAgent = () =>
  isBrowser() && window.navigator.userAgent.toLowerCase();
```
 ...
 */
const isChrome = () =>
  getUserAgent() && /chrome\/\d+/.test(getUserAgent()) && !isEdge();
```
...
const makeObj = (str, expectLowerCase) => {
  ...
  return expectLowerCase ? (val) => obj[val.toLowerCase()] : (val) => obj[val];
};
```
